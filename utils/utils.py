...
def box_iou(box1, box2):
    ...
    #print("INTER", torch.min(box1[:, None, 2:], box2[:, 2:]) - torch.max(box1[:, None, :2], box2[:, :2])) #.clamp(0).prod(2))
    
    #print("MIN", torch.min(box1[:, None, 2:], box2[:, 2:]))
    #print("MAX", torch.max(box1[:, None, :2], box2[:, :2]))
    #print("VALS", box1[:, None, 2:], box2[:, 2:])
...
def cooler_nms(bounding_boxes, confidence_score, threshold):
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #i = torchvision.ops.boxes.nms(boxes.cpu(), scores.cpu(), iou_thres)
        i = cooler_nms(boxes.cpu().numpy(), scores.cpu().numpy(), iou_thres) #torchvision.ops.boxes.nms(boxes, scores, iou_thres)
...
def asdf_non_max_suppression(prediction, conf_thres=0.2, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    """
    Performs  Non-Maximum Suppression on inference results
    Returns detections with shape:
        nx6 (x1, y1, x2, y2, conf, cls)
    """
    prediction = torch.abs(prediction)  # Stuff can get negative values. This is a safeguard but really bad. 
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        print(boxes)
        print(scores)
        i = cooler_nms(boxes.cpu().numpy(), scores.cpu().numpy(), iou_thres)
        ...
        output[xi] = x[i]
        if (time.time() - t) > time_limit:
            print("Time limit exceeded. Exiting NMS.")
            break  # time limit exceeded
